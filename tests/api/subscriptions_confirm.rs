...
async fn the_link_returned_by_subscribe_returns_a_200_if_called() {
    ...
    let conformation_links = app.get_confirmation_links(email_request);

    // Act
    let response = reqwest::get(conformation_links.html).await.unwrap();
...
async fn clicking_on_the_confirmation_link_confirms_a_subscriber() {
    ...
    // Act
    reqwest::get(conformation_links.html)
        .await
        .unwrap()
        .error_for_status()
        .unwrap();
